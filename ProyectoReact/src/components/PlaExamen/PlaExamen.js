...
function PlaExamen() {
    ...
    useEffect(() => {
        ...
        .then(response => {
            const reservasActualizadas = response.data;

            // Actualizar el estado de las salas con las reservas correspondientes
            const salasActualizadas = salas.map(sala => {
                const diasActualizados = sala.dias.map(dia => {
                    const reservasDia = reservasActualizadas.filter(reserva => reserva.sala === sala.id && reserva.fecha === dia.fecha);
                    if (reservasDia.length > 0) {
                        dia.modulos = dia.modulos.map((modulo, index) => {
                            const reservaModulo = reservasDia.find(reserva => reserva.modulos === index + 1);
                            if (reservaModulo) {
                                return {
                                    ...modulo,
                                    estado: 'reservado',
                                    evento: reservaModulo.evento,
                                    seccion: reservaModulo.seccion,
                                    asignatura: reservaModulo.asignatura,
                                    docente: reservaModulo.docente,
                                };
                            }
                            return modulo;
                        });
            ...
            setSalas(salasActualizadas); // Actualiza el estado de las salas con las reservas aplicadas
        })
    ...
    const reservarModulos = () => {
        ...
        // Preparar la información de la reserva
        const reservas = modulosSeleccionados.map(modulo => ({
            sala: selectedSala.id,  // Asegúrate de que esté usando el ID correcto de la sala
            examen: selectedExam.id,  // Utiliza el ID del examen seleccionado
            modulos: modulo.numero,  // Número de módulo seleccionado
            fecha: modulo.fecha,  // Fecha de la reserva
        ...
        // Enviar cada reserva al backend
        Promise.all(
        ...
        .then(responses => {
            console.log('Reserva(s) confirmada(s):', responses);
            alert("Reservas confirmadas exitosamente");

            // Actualizar las salas o cualquier estado necesario aquí después de la reserva
            setSalas(prevSalas => prevSalas.map(sala =>
                sala.id === selectedSala.id ? { ...sala, dias: actualizarDiasConReservas(sala.dias, reservas) } : sala
            ));
        })
        .catch(error => {
        ...
        // Función auxiliar para actualizar la estructura de días con las reservas realizadas
        const actualizarDiasConReservas = (dias, reservas) => {
            return dias.map(dia => {
                const reservasDelDia = reservas.filter(reserva => reserva.fecha === dia.fecha);
                const modulosActualizados = dia.modulos.map((modulo, index) => {
                    const reservaEncontrada = reservasDelDia.find(reserva => reserva.modulos === index + 1);
                    if (reservaEncontrada) {
                        return {
                            ...modulo,
                            estado: 'reservado',
                            evento: reservaEncontrada.evento,
                            seccion: reservaEncontrada.seccion,
                            asignatura: reservaEncontrada.asignatura,
                            docente: reservaEncontrada.docente,
                        };
                    }
                    return modulo;
                });
                                ...
                                return (
                                    <td key={`${dia}-${indexModulo}`}>
                                        {modulo.estado === 'reservado' ? (
                                            ...
                                            <div>
                                                <button onClick={() => seleccionarModulo(dia, fechasDeLaSemana[dia], indexModulo + 1)}>
                                                    Seleccionar
                                                </button>
                                            </div>
                                        )}
